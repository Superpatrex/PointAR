    ...
    if results.boxes is not None and len(results.boxes) > 0:
        for box in results.boxes:
            ...
            if label == "lamp":
                ...
                if lamp_roi.size > 0:
                    try:
                        # --- NEW: Use pyzbar for robust detection ---
                        # 1. Convert to grayscale for better contrast
                        gray_roi = cv2.cvtColor(lamp_roi, cv2.COLOR_BGR2GRAY)
                        
                        # --- NEW: Apply preprocessing to make QR code pop ---
                        # Use adaptive thresholding to create a clean B&W image
                        # This is highly effective against glare and bad lighting
                        thresh_roi = cv2.adaptiveThreshold(
                            gray_roi, 255, 
                            cv2.ADAPTIVE_THRESH_GAUSSIAN_C, 
                            cv2.THRESH_BINARY, 
                            11, 2 # Block size and C value (tweak if needed)
                        )
                        
                        # 2. Decode the preprocessed image using pyzbar
                        barcodes = pyzbar_decode(thresh_roi)
                        
                        if not barcodes:
                            # If it fails, try decoding the plain grayscale image
